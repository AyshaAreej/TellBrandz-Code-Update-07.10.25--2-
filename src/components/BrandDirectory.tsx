...
const BrandDirectory: React.FC<BrandDirectoryProps> = ({
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // 1. Get tells with brand information
        const { data: tellsData } = await supabase
          .from("tells")
          .select("id, brand_id, brand_name, tell_type");

        // 2. Get brand table data
        const { data: brandsData } = await supabase
          .from("brands")
          .select("id, name, logo_url, category, location, established");

        // 3. Get comments (reviews) with tell relationships
        const { data: commentsData } = await supabase
          .from("comments")
          .select("id, tell_id, content, created_at");

        // 4. Get likes with tell relationships
        const { data: likesData } = await supabase
          .from("likes")
          .select("id, tell_id, user_id, created_at");
  ...
  const filteredBrands = brands
    .filter((brand) => {
      ...
      const matchesRating =
        brand.rating >= advancedFilters.ratingRange[0] &&
        brand.rating <= advancedFilters.ratingRange[1];
      const matchesReviews =
        brand.reviews >= advancedFilters.reviewRange[0] &&
        brand.reviews <= advancedFilters.reviewRange[1];
      const matchesLocation =
        advancedFilters.locations.length === 0 ||
        advancedFilters.locations.some((loc) =>
          brand.location.toLowerCase().includes(loc.toLowerCase())
        );
    ...
    .sort((a, b) => {
      switch (sortBy) {
        ...
        case "engagement":
          return b.engagementScore - a.engagementScore;
        case "tells":
          return b.tellsCount - a.tellsCount;
        case "satisfaction":
        ...
        <div className="container mx-auto px-2 sm:px-4 py-4 sm:py-8">
          <div className="mb-4 sm:mb-8">
            ...
            <p className="text-sm sm:text-base text-gray-600 mb-4 sm:mb-6">
              Discover, explore, and compare brands across various categories
            </p>
                  ...
                  {filteredBrands.map((brand) => (
                    ...
                    >
                      ...
                      >
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            >
                              {brand.category.length > 12
                                ? brand.category.substring(0, 12) + "..."
                                : brand.category}
                            </Badge>
                          </div>
                        </div>
                      ...
                      >
                        ...
                        <Link to={`/brand/${createSlug(brand.name)}`}>
                          <Button
                            variant="outline"
                            size="sm"
                  ...
                  <div className="text-center py-12 sm:py-20 bg-white rounded-lg shadow-sm">
                    ...
                    <div className="space-y-4">
                      ...
                      <p className="text-sm text-gray-500">
                        Help build the directory by sharing your brand
                        experiences
                      </p>
                    </div>
                  </div>
                ...
                <div className="space-y-4 sm:space-y-6">
                  ...
                  {favorites.length === 0 ? (
                    <div className="text-center py-12 sm:py-20 bg-white rounded-lg shadow-sm">
                      ...
                      <p className="text-gray-600 mb-8 max-w-md mx-auto">
                        Start exploring brands and click the heart icon to add
                        them to your favorites. Build your personalized
                      ...
                      <div className="space-y-4">
                        ...
                        <p className="text-sm text-gray-500">
                          Discover brands and save your favorites for quick
                          access
                        </p>
                      </div>
                    </div>
