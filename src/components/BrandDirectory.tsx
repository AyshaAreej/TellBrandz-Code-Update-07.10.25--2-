 ...
 useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const brandsWithStats = Object.keys(brandStats).map((brandName, i) => {
          const stats = brandStats[brandName];
          ...
          if (stats.totalTells > 0) {
            const blastPercentage = (stats.brandBlasts / stats.totalTells) * 100;
          ...
          if (finalRating === 0 && stats.totalTells > 0) {
            ...
            finalRating = Math.round((satisfactionScore / 100) * 4 + 1 * 10) / 10;
          }

          return {
            id: brandData?.id || `brand-${i}`,
            name: brandName,
            logo: brandData?.logo_url ? `<img src="${brandData.logo_url}" alt="${brandName}" class="w-8 h-8 object-contain" />` : '🏢',
            ...
            type: stats.brandBlasts > stats.brandBeats 
              ? "Mostly Positive" 
            ...
            // Trend indicator
            trend: stats.brandBlasts > stats.brandBeats ? "up" : 
                   stats.brandBeats > stats.brandBlasts ? "down" : "stable"
          };
        }).sort((a, b) => {
  ...
  const categories = [
    ...
    'Manufacturing', 'Transportation', 'Food & Beverage', 'Technology',
    'Healthcare', 'Retail', 'General'
  ];
